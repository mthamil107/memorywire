"""Render the LinkedIn-format AMP explainer GIF using Pillow.
...
six storyboard beats in ~6 seconds, then loops:
  ...
  4. AMP layer inserts itself between agent/boxes (3.0-4.0s)
...
INDIGO = (59, 91, 181)            # AMP accent
INDIGO_LIGHT = (220, 226, 245)    # AMP fill
...
# Five-framework palette — desaturated so the AMP indigo stays visually
...
def _load_font(candidates: list[str], size: int) -> ImageFont.ImageFont:
    for p in candidates:
        try:
            ...
            continue
    # PIL's built-in bitmap font ignores size — last-resort safety net.
    try:
...
def draw_framework_box(
    ...
    cleanly against the rounded interior — drawing diagonals straight on
...
# ---------------------------------------------------------------------------
# Beat renderers — each returns a fully composed PIL Image for one frame.
# ---------------------------------------------------------------------------
...
class Layout:
    ...
    # AMP horizontal layer between agent and boxes
    AMP_TOP = 430
    AMP_BOT = 510
    AMP_LEFT = 140
    AMP_RIGHT = WIDTH - 140
...
def _draw_beat1(
    ...
    # already shows readable text — LinkedIn often previews frame 0 as a
...
def _draw_beat2(
    ...
    # downward fan of arrows or the AMP arrow in later beats.
    draw_text_anchored(
        d, "Your agent",
...
# --- Beat 4: AMP layer inserts itself ----------------------------------------
...
def _draw_beat4(
    ...
    # AMP layer grows in over the first few frames.
    ...
    # Agent -> AMP single arrow appears late.
    if p > 0.7:
        arr_p = (p - 0.7) / 0.3
        sx = Layout.AGENT_CX
        sy = Layout.AGENT_CY + Layout.AGENT_R + 4
    ...
    # AMP -> 5 boxes arrows appear last.
    if p > 0.85:
        for i in range(5):
...
def _draw_beat5(
    ...
    # Persist beat 4 backdrop (agent, AMP, boxes, arrows) at reduced contrast
    ...
    # Body — show a diff card for a pending memory write.
...
def _draw_beat6(
    ...
    # Full beat-5 composition stays — including the governance panel — for
    ...
    cta2 = "github.com/mthamil107/agent-memory-protocol"
...
def render() -> Path:
    fonts = _build_fonts()
    out_path = Path(__file__).resolve().parent / "amp-explainer.gif"
    ...
    # the surviving frame — preserving the full 6.5s loop length while
