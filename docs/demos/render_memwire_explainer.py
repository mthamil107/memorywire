"""Render the LinkedIn-format memwire explainer GIF using Pillow.
...
six storyboard beats in ~6 seconds, then loops:
  ...
  4. memwire layer inserts itself between agent/boxes (3.0-4.0s)
...
INDIGO = (59, 91, 181)            # memwire accent
INDIGO_LIGHT = (220, 226, 245)    # memwire fill
...
# Five-framework palette â€” desaturated so the memwire indigo stays visually
...
class Layout:
    ...
    # memwire horizontal layer between agent and boxes
    AMP_TOP = 430
    AMP_BOT = 510
    AMP_LEFT = 140
    AMP_RIGHT = WIDTH - 140
...
def _draw_beat2(
    ...
    # downward fan of arrows or the memwire arrow in later beats.
    draw_text_anchored(
        d, "Your agent",
...
# --- Beat 4: memwire layer inserts itself ----------------------------------------
...
def _draw_beat4(
    ...
    # memwire layer grows in over the first few frames.
    ...
    # Agent -> memwire single arrow appears late.
    if p > 0.7:
        arr_p = (p - 0.7) / 0.3
        sx = Layout.AGENT_CX
        sy = Layout.AGENT_CY + Layout.AGENT_R + 4
    ...
    # memwire -> 5 boxes arrows appear last.
    if p > 0.85:
        for i in range(5):
...
def _draw_beat5(
    ...
    # Persist beat 4 backdrop (agent, memwire, boxes, arrows) at reduced contrast
...
def _draw_beat6(
    ...
    cta2 = "github.com/mthamil107/memwire"
...
def render() -> Path:
    fonts = _build_fonts()
    out_path = Path(__file__).resolve().parent / "memwire-explainer.gif"
