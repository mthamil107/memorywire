...
3. **One ask per DM.** Either "would you look at the spec" OR
   "would you accept an adapter PR" — never both.
...
## Tier 1 — Memory framework maintainers (file PRs first, then DM)
...
### 1. Charles Packer — Letta (formerly MemGPT)
...
- Ask: would Cognee expose per-record IDs (the v0.2 spec-
  tightening signal from the conformance suite §5.3) — and
  if not, what's the workaround for the synthetic `cog:`
  ID problem.

### 4. Daniel Chalef — Zep / Graphiti
- Handle: `@danielchalef` (X)
- Why: founder of Zep + Graphiti. AMP's RRF k=60 fusion is
  the same recipe Graphiti uses.
- Opener hook: Graphiti's temporal-knowledge-graph paper
  or a recent Zep release.
- Ask: would you accept an `amp.store.zep_adapter` PR for
  v0.2 — the conformance suite would gain a 6th column.

---

## Tier 2 — Foundation-model memory teams

These are the highest-leverage hire-me targets if the OSS
path doesn't convert. Per `docs/kickoff/FUTURE.md`.

### 5. Anthropic — Claude Memory team
...
- Ask: feedback on AMP's positioning relative to MCP, since
  Anthropic is the MCP author. AMP is downstream / extension.
...
- Ask: is AMP's 4-type taxonomy compatible with whatever
  ChatGPT memory's internal type system looks like, and
  would a public-facing protocol be useful.

### 7. Microsoft — Azure AI Foundry Memory team
...
- Ask: feedback on AMP's governance UI as a model for
  what an enterprise-tier memory audit surface should
  look like.

### 8. Google — ADK Memory team
...
## Tier 3 — Standards-body engagement
...
- Ask: would the MCP-WG accept a memory-extension proposal
  citing AMP as the reference impl, or is there a different
  shape they'd prefer.

### 10. IETF — applicable working group
- AMP's intended IETF home is either
  `httpapi` (REST-shaped JSON-Schema APIs) or `core` (constrained
  applications). Find the WG chair via the IETF datatracker.
- Opener hook: nothing — IETF DMs are usually cold and
...
- Ask: would AMP's v0.5 spec be in scope for an Internet-
  Draft submission to your WG.

### 11. W3C — Community Group sponsor
...
- Ask: would you sponsor an "Agent Memory" Community
  Group with AMP as the founding submission.

---

## Tier 4 — Adjacent ecosystem (lower priority, higher count)

These are LOW-LIKELIHOOD targets but the bar is also lower
(any one of them replying is a small win).

### 12. HuggingFace — community OSS team
- Acquires OSS tools that fit the Hub.
- Opener hook: a recent HF release.
- Ask: would AMP fit as a `pip install huggingface-amp`
...
## Tier 5 — Researchers + thinkers (no ask, just signal)

These five are not commercial targets but their public
reaction (or silence) is a strong signal. Don't ask
anything — just send the arXiv link with one line of
context.

### 16. Andrej Karpathy — `@karpathy`
- Has tweeted about memory + agents repeatedly. A like is
  signal; a quote-RT is gold.

### 17. Sebastian Raschka — `@rasbt`
- LLM systems writer; has covered agent infra in the past.

### 18. Jeremy Howard — `@jeremyphoward`
- fast.ai founder; pragmatic-systems sensibility matches AMP.

### 19. Simon Willison — `@simonw`
- LLM bloggerati; writes detailed posts on the agent
  ecosystem. AMP fits his beat exactly.
...
Saw <specific reference to their published work — paper / blog / tweet from last 4 weeks>.

I just released an arXiv preprint that intersects with what you're shipping: AMP, a vendor-neutral wire format for agent memory operations. The reference implementation includes <ONE OF: a Letta adapter / a mem0 adapter / a Cognee adapter / a Zep adapter / nothing yet for your work but should — drop in the specific surface to their work here>.

The contribution is a protocol + reference impl + governance UI; the framing is "the cross-vendor layer above storage", positioned to compose with MCP. Honest framing on novelty — not an algorithmic contribution, packaging + standards positioning.

arXiv: https://arxiv.org/abs/<ARXIV_ID>
Code: https://github.com/mthamil107/agent-memory-protocol
...
30 days, count the engaged rows — that's your week-4 success
metric per `docs/kickoff/PROJECT-PLAN.md`.

Target metric (per the kickoff):

> ≥1 of mem0/Letta/Cognee accepted (or substantively engaged on)
> the adapter PR; ≥3 first-round interviews scheduled with target
> memory teams; ≥1 spec comment from someone at
