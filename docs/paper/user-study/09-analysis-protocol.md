...
Computed per the procedure in `06-nasa-tlx.md §3`: each of the six
...
- **Per-task success rate** — fraction of participants meeting each
  task's success criterion in `05-tasks.md` within the 3-minute budget.
  Reported per task with 95% CI (Wilson interval).
- **Per-task completion time** — seconds from task start (researcher
  starts timer) to task end (success or timeout). Reported as mean,
  median, IQR, 95% CI per task. Time-to-timeout (180 s) is recorded
  for failures.
- **Per-task Likert confidence (L1)** — mean and 95% CI per task on
  the 1-7 scale.
- **Overall Likert items (L2-L7)** — mean and 95% CI per item.
- **NASA-TLX per-dimension** — mean and 95% CI per dimension; also
...
- Correlation between **L4 production-trust** and **mental-demand
  ...
  production trust. Reported as Spearman ρ with 95% CI.
...
- **Means with 95% CIs:** parametric (t-distribution) when n ≥ 15 and
...
- **Correlations:** Spearman ρ with bootstrap 95% CIs.

The paper will explicitly state in §5.5.3 that the sample size is too
...
problems."* We target n = 15 — well past the Nielsen elbow at 5,
...
**Acceptance threshold:** kappa ≥ **0.61** ("substantial agreement"
...
agreement. Kappa is recomputed and reported transparently in §5.5.4.

### 3.4 Coder selection

- **Coder 1:** the PI.
- **Coder 2:** `[NAME OR ROLE — e.g., a colleague external to the AMP
...
The paper's §5.5 is structured as:

- **§5.5.1 Method** — lifted from `04-study-protocol.md` and
  `05-tasks.md`.
- **§5.5.2 Participants** — n; aggregate stats from
  `03-pre-study-survey.md`.
- **§5.5.3 Quantitative results** — TLX table; per-task table; Likert
  table; figure with bar chart + radar from `analyze.py`.
- **§5.5.4 Qualitative themes** — 3-7 themes with representative
...
serious usability problems, those results appear in §5.5 with the same
...
Any deviations from this protocol — e.g. recruitment cut short at
n = 10, a task modified mid-study, an additional analysis performed —
will be logged here at the time they happen, with a note explaining
why. The deviations log is reported in §5.5.5.
