# IRB Application — AMP Governance UI Usability Study
...
> usability) covers this protocol — most U.S. R1 university IRBs do.
...
- **Study title:** Usability evaluation of the AMP Governance UI: a
...
(d) participant data will be handled per the data-storage plan in §7.
...
The Agent Memory Protocol (AMP) is an open-source wire format for
agent-memory operations. Alongside the protocol AMP ships a
**governance UI** — a web interface where a human operator reviews
...
flow give a software engineer or ML practitioner — somebody who has not
previously used the UI — enough information to make correct approve /
reject decisions with reasonable cognitive workload? The study will be
reported as §5.5 of the AMP paper. There is no comparison condition;
...
(2000) — usability studies converge on the major problems at
...
1. The investigator's professional network on LinkedIn (one post + DMs
   to candidates who pass the screening criteria in §3.1).
2. The AMP open-source project's GitHub Discussions board (one pinned
...
- Anyone who has previously contributed to the AMP repository
...
software-UX study per 45 CFR 46.117(c)(1)(ii) — the consent form is
...
The study presents **minimal risk** in the meaning of 45 CFR 46.102(j) —
...
| Loss of confidentiality (recording leaks) | Very low | Moderate | Recordings stored encrypted; access limited to investigator; deleted after analysis (see §7). |
...
- Quantitative data is stored as CSV files in an encrypted directory on
  ...
  PROVIDER]`). Access limited to study personnel listed in §1.
...
- Aggregate statistics and de-identified quotations appear in the AMP
  paper.
...
compensation is funded from `[FUNDING SOURCE — DEPARTMENTAL DISCRETIONARY
FUND / GRANT NUMBER]`.

Total compensation budget: 15 × USD 15 = **USD 225**.
...
- Reports to the PI or has any other coercion-creating relationship to
  the study team.
- Previously contributed to the AMP open-source repository (commits,
...
The PI is the author and primary maintainer of the AMP open-source
...
participates per §11), (b) reporting all results — positive and
negative — in the paper without selection, and (c) releasing raw data
...
across the 15 transcripts. Per Landis and Koch (1977), kappa ≥ 0.61
...
consensus-coding pass. The second coder is `[NAME OR ROLE — e.g.
a colleague external to the AMP project, recruited specifically for
this study]`.
...
- Nielsen, J. (2000). *Why you only need to test with 5 users.* Nielsen
  Norman Group.
- 45 CFR 46 — U.S. Federal Policy for the Protection of Human Subjects
  ("Common Rule").
...
- `01-consent-form.md` — informed-consent form.
- `02-recruitment.md` — recruitment email + screening criteria.
- `03-pre-study-survey.md` — pre-study demographic survey.
- `04-study-protocol.md` — researcher session script.
- `05-tasks.md` — the five governance-UI tasks.
- `06-nasa-tlx.md` — NASA-TLX questionnaire.
- `07-post-task-survey.md` — post-task survey instrument.
- `08-debrief.md` — debrief script.
- `09-analysis-protocol.md` — pre-registered analysis plan.
