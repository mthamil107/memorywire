...
The evaluation has four parts. \Cref{sec:eval-micro} reports a microbenchmark on a labelled corpus to establish baseline recall and latency. \Cref{sec:eval-adversarial} reports an adversarial-fusion experiment that quantifies the robustness premium RRF provides over MAX and weighted fusion under a 1-of-$N$ rank-0 injection attack. \Cref{sec:eval-conformance} reports a cross-adapter conformance suite that empirically validates the protocol's vendor-neutrality claim. \Cref{sec:eval-recovery} evaluates recovery from memory poisoning using an external benchmark. \Cref{sec:eval-threats} enumerates threats to validity.
