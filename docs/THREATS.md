...
> Version: v0 (draft) — 2026-05-27
> Scope: the AMP protocol, reference implementation in `src/amp/`,
> and governance UI in `ui/`.
> Out of scope: third-party backend security (mem0, Letta, Cognee,
> Postgres) — those have their own threat models.
...
honest about residual risk. It is descriptive, not prescriptive — the
...
agent process (trusted)
  └─ in-process → AMP SDK (Memory facade)
        └─ in-process → router (src/amp/router.py)
              └─ per-adapter (may cross network) → sqlite-vec | mem0 |
                           ...
                           governance UI (ui/, separate process)
                             - BearerAuthMiddleware (opt-in AMP_UI_TOKEN)
...
- **Agent process: trusted.** Whatever calls `Memory.remember()` is
  ...
  the host application (`docs/spec/v0.md §1`).
...
- **Backends: partially trusted.** Per-store failures cannot crash the
  router (`return_exceptions=True` fan-out at `src/amp/router.py:261-264,
  353-356`), but a malicious backend *can* influence fused recall (§2.3).
- **Governance UI: separate trust zone** with opt-in bearer auth +
  CSRF. Without `AMP_UI_TOKEN` it is unauthenticated — fine for a
  single operator on localhost, not for a public bind.
- **Audit log: append-only by convention.** Schema in
  `docs/kickoff/ARCHITECTURE.md §3`; sole writer is
  `SqliteVecStore._audit` (`src/amp/store/sqlite_vec.py:401-424`).
...
**Capability:** submits `remember()` calls — as the agent itself
...
surfaces — the memory analogue of prompt injection (OWASP LLM-01,
CWE-20). **Preconditions:** can influence any string the agent passes
through `remember()`. AMP cannot tell a real fact from a planted one.

### 2.2 Recall exfiltration

**Capability:** issues `recall()` against an agent's router — directly
...
with data. `k` defaults to 5; the schema caps at 1000 (`docs/spec/v0.md §3.2`).

### 2.3 Poisoned backend in RRF fusion

**Capability:** controls one of the N child stores in a `MemoryRouter`
— a compromised hosted mem0 instance, a malicious adapter fork, or a
...
(`docs/spec/v0.md §3.1`) stages the row behind the
`PENDING_APPROVAL_DELETED_AT = -1` sentinel in `memories.deleted_at`
(`src/amp/store/sqlite_vec.py:88-98, 440`, commit `c828d76`). All recall
...
(`src/amp/store/sqlite_vec.py:505-512`), so a poisoned memory is traceable
to the call that planted it.

**Residual risk:** default `approval_required` is false; AMP cannot tell
a real fact from a planted one — it only guarantees that what the agent
wrote is what the agent reads back. Prompt-injected calls from a trusted
agent are out of scope (§5).

**v0.2 hardening:** a `privacy_intent` block on `RememberRequest`
(`docs/spec/v0.md §10`) so operators can require approval by `source`,
...
adapter SQL layer — every recall path in `SqliteVecStore` filters
`WHERE agent_id = ? AND deleted_at IS NULL`, and `agent_id` is required
by the spec (`docs/spec/v0.md §3.2`). `recall()` is itself audited
...
— the governance channel covers `remember` / `forget` / `merge` only
(`docs/spec/v0.md §6`).
...
**Current mitigation:** RRF is **score-independent** by construction —
`_fusion_contribution` uses `1 / (rrf_k + rank)` (`src/amp/router.py:428-429`),
...
contributes at most `1/60 ≈ 0.0167` per item, while a two-store consensus
item scores `≥ 1/60 + 1/61 ≈ 0.0330`. `MAX` and `WEIGHTED` fusion *are*
score-sensitive — operators picking them accept more backend trust
(documented at `src/amp/router.py:415-444`). Per-store failures are logged
but do not abort the operation (`src/amp/router.py:361-368`).
...
by reporting it at rank 0. There is no cross-store identity proof —
...
(`src/amp/store/sqlite_vec.py:401-424`); it only performs `INSERT INTO
audit_log(...)`. No UPDATE or DELETE against `audit_log` exists anywhere
in `src/amp/` or `ui/src/amp_ui/`. Every governance action emits a
dedicated audit row with the reviewer's identity — `approve()`,
...
**Residual risk:** SQLite is a single file with filesystem semantics —
anyone with write access can `DELETE FROM audit_log` or mutate rows. AMP
...
- `approve()` — `UPDATE memories SET deleted_at = NULL WHERE id = ? AND
  agent_id = ? AND deleted_at = ?` (`ui/src/amp_ui/services.py:502-511`).
- `reject()` — same triple-key guard (`ui/src/amp_ui/services.py:551-559`).
- `apply_co_memorize()` — both the forget branch
...
- On any mismatch (row missing, wrong agent, already forgotten),
  ...
  "already forgotten by a concurrent process" — the response itself does
  not leak which condition fired (`ui/src/amp_ui/services.py:902-911`).

The pending sentinel is a shared public constant
(`PENDING_APPROVAL_DELETED_AT`, `src/amp/store/sqlite_vec.py:91`) which
...
no per-operator role or per-`agent_id` ACL — a user with the bearer can
...
every request behind a bearer token when `AMP_UI_TOKEN` is set
(`ui/src/amp_ui/middleware.py:74-103`), accepting either an
`Authorization: Bearer` header or an `amp_ui_session` cookie, with
...
(`src/amp/procedural.py:57-59`), explicitly rejecting `before`, `after`,
and `prepare` — the three pytransitions keys that resolve dotted-string
callbacks via `__import__` and would otherwise let
`{"before": "os.system"}` execute arbitrary code at trigger time
(`src/amp/procedural.py:44-56`). `conditions` and `unless` strings are
...
(`src/amp/procedural.py:122-136`). `ProcedureRunner._expand_transitions`
strips disallowed keys *at construction time* even if a caller bypasses
`from_dict` (`src/amp/procedural.py:283-312`). Both layers shipped in
`c828d76`.
...
(`ui/src/amp_ui/middleware.py:151`) — a documented v0 trade-off: a
...
**v0.2 hardening:** `AMP_UI_CSRF_SECRET` pinning (spec-gap in `c828d76`);
...
Real threats AMP v0 does **not** address, by design:

- **Insider attacks on the audit DB** — anyone with filesystem write
  access can rewrite history (see §3.4 residual). Mitigated only by
  deployment hygiene.
- **Side-channel timing attacks on recall** — latency varies with `k`,
...
- **Prompt injection in the calling agent** — AMP is downstream of the
  LLM. If the agent is fooled into `remember("Alice is in the
  building", confidence=1.0)`, AMP faithfully stores and recalls it
  (see §5).
- **Supply-chain attacks on the wheel** — PyPI publishing uses OIDC
  trusted publishing per `docs/kickoff/ARCHITECTURE.md §1` but there
  is no in-band signature verification at SDK load. Mitigated by
  `pip`'s standard hash-pinning.
- **Backend transport security** — `mem0` / `Letta` / `pgvector`
  adapters delegate TLS and auth to their upstream SDKs. AMP does not
...
Threats inherent to the agent / LLM, not to AMP. AMP makes them
...
- An LLM issues `recall()` with an over-broad query and leaks results
  into a user-visible response. AMP returned the hits correctly; the
  agent leaked them.
- An LLM stores content it should not (e.g. a card number) via
  `remember()`. AMP's `metadata` is opaque; there is no PII detector.
- An LLM picks the wrong `MemoryType`. AMP enforces type at write time
...
features land — once audit-row hashing ships (v0.2 hardening §3.4),
that bullet moves into §3.4's "Current mitigation".

---

## 7. Reporting

Suspected vulnerabilities should be reported privately — see
[`SECURITY.md`](../SECURITY.md) at the repo root. Use the GitHub
Security tab's "Report a vulnerability" feature on the
[agent-memory-protocol repository](https://github.com/mthamil107/agent-memory-protocol).
...
- The security-review agent run that surfaced the five issues fixed in
  ...
  co-memorize across `agent_id` (security, §3.5); (3) UI lacks auth +
  CSRF (security, §3.6a); (4) pytransitions RCE via crafted procedural
  memory before/after keys (security, §3.6b); (5) UI 500 on fresh DB
  (correctness).
- [`SECURITY.md`](../SECURITY.md) — reporting policy.
- [`docs/spec/v0.md`](./spec/v0.md) — protocol surface (§§3, 6, 7
  cited throughout this document).
- [`docs/kickoff/ARCHITECTURE.md`](./kickoff/ARCHITECTURE.md) — storage
  schema (§3) and audit-log shape.

External standards we map to:

- [OWASP Top 10 (2021)](https://owasp.org/Top10/): A01 (Broken Access
  Control, §3.5), A03 (Injection, §3.6b), A04 (Insecure Design, §3.6),
  A07 (Auth Failures, §3.6a), A09 (Logging Failures, §3.4).
- [OWASP Top 10 for LLM Applications](https://owasp.org/www-project-top-10-for-large-language-model-applications/):
  LLM-01 (Prompt Injection, §3.1 and §5), LLM-03 (Training/Data
  Poisoning analogue, §3.1).
- CWE references: CWE-20 (§3.1), CWE-94 (§3.6b), CWE-117 / CWE-778
  (§3.4), CWE-200 (§3.2), CWE-345 (§3.3), CWE-639 (§3.5), CWE-862
  (§3.6a).
- MITRE ATT&CK mapping (informal): T1565.001 "Stored Data Manipulation"
  (§3.4), T1078 "Valid Accounts" (§3.5), T1059 "Command and Scripting
  Interpreter" (§3.6b).
