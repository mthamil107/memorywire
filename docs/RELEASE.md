...
1. Open **GitHub repo Ã¢â€ â€™ Settings Ã¢â€ â€™ Actions Ã¢â€ â€™ General**.
...
2. Open **"Your projects" Ã¢â€ â€™ Trusted Publisher Management**
   (or visit <https://pypi.org/manage/account/publishing/>).
3. Click **"Add a new publisher" Ã¢â€ â€™ GitHub** and paste:
...
In **GitHub repo Ã¢â€ â€™ Settings Ã¢â€ â€™ Environments**, create two environments:

- **`pypi`** Ã¢â‚¬â€ for production releases. Optionally add `mthamil107` as a
  required reviewer so every production publish needs a final click.
- **`testpypi`** Ã¢â‚¬â€ for pre-release dry runs; no reviewers required.
...
4. **The GitHub release publication** triggers `release.yml`, which:
   ...
   that tag-push event from firing downstream workflows Ã¢â‚¬â€ so we listen
