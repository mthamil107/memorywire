...
SCHEMAS_ROOT = REPO_ROOT / "src" / "amp" / "schemas"
...
# in the spec Editor's notes and modelled in ``amp.models``.
...
def test_example_response_roundtrip(operation: str, example_path: Path) -> None:
    ...
    All five operations have response models in :mod:`amp.models` (the three
