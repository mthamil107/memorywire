"""Starlette application factory for the memwire Governance UI.
...
app against a temporary database. Production use is via :mod:`memwire_ui.__main__`,
...
from memwire_ui.middleware import (
...
def create_app(
    *,
    ...
    db_path:
        ...
        If ``None``, fall back to the ``MEMWIRE_UI_DB_PATH`` env var, then to
        ``./memwire-cli.db``. Pass ``":memory:"`` for an ephemeral app (only
    ...
    token:
        ...
        clients) or the ``memwire_ui_session=<token>`` cookie (browsers).
...
def _resolve_db_path(db_path: str | Path | None) -> str:
    ...
    env = os.environ.get("MEMWIRE_UI_DB_PATH")
    if env:
        return env
    return str(Path("./memwire-cli.db"))
