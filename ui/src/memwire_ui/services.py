"""Data-access layer for the memwire Governance UI.

The UI is a thin renderer over the same SQLite database the
:class:`memwire.store.sqlite_vec.SqliteVecStore` adapter writes to. This module
...
def _connect(db_path: str | Path) -> sqlite3.Connection:
    """Open a fresh connection to the memwire SQLite database.
...
def ensure_schema(db_path: str | Path) -> None:
    ...
    ``python -m memwire_ui`` against an empty ``./memwire-cli.db``), nothing has
