...
Reads ``MEMWIRE_UI_DB_PATH``, ``MEMWIRE_UI_AGENT_ID``, ``MEMWIRE_UI_HOST``,
``MEMWIRE_UI_PORT``, ``MEMWIRE_UI_TOKEN``, and ``MEMWIRE_UI_CSRF_SECRET`` from the
environment, builds the Starlette app via :func:`amp_ui.app.create_app`,
and hands it to ``uvicorn.run``.

``MEMWIRE_UI_TOKEN`` Ã¢â‚¬â€ when set, gates every request behind that bearer
token. **Required** when ``MEMWIRE_UI_HOST`` resolves to any non-loopback
...
``MEMWIRE_UI_ALLOW_UNAUTHENTICATED_PUBLIC=1`` to explicitly opt out of the
check (e.g. demo-only deployments fronted by their own auth layer).

``MEMWIRE_UI_CSRF_SECRET`` Ã¢â‚¬â€ base64-encoded HMAC secret (>= 16 raw bytes)
...
CSRF_SECRET_ENV_VAR = "MEMWIRE_UI_CSRF_SECRET"
_MIN_CSRF_SECRET_BYTES = 16

# Hostnames that bind only to the local machine Ã¢â‚¬â€ safe to run without
# MEMWIRE_UI_TOKEN because nothing off-box can reach the listener.
...
def _assert_safe_public_config(
    ...
    ----------
    host:
        The value of ``MEMWIRE_UI_HOST`` (or equivalent CLI arg). Anything
    ...
    token:
        The value of ``MEMWIRE_UI_TOKEN``. ``None`` or empty string means
        "no bearer auth" and the UI's middleware becomes a no-op.
    allow_unauth:
        Truthy when ``MEMWIRE_UI_ALLOW_UNAUTHENTICATED_PUBLIC=1`` is set.
    ...
    if is_loopback or has_token or allow_unauth:
        return
    print(
        "amp_ui: refusing to start a public-bound UI without MEMWIRE_UI_TOKEN.\n"
        f"MEMWIRE_UI_HOST is set to {host!r}. Either:\n"
        "  - Set MEMWIRE_UI_HOST=127.0.0.1 (loopback-only), OR\n"
        "  - Set MEMWIRE_UI_TOKEN=<a-bearer-token-string> via env or "
        "`fly secrets set`.\n"
        "See ui/README.md for token generation.\n"
        "To explicitly opt out of this check (e.g. a demo behind your "
        "own auth layer), set MEMWIRE_UI_ALLOW_UNAUTHENTICATED_PUBLIC=1.",
        file=sys.stderr,
    )
    raise SystemExit(1)
...
def main() -> None:
    ...
    host = os.environ.get("MEMWIRE_UI_HOST", "127.0.0.1")
    port = int(os.environ.get("MEMWIRE_UI_PORT", "8765"))
    agent_id = os.environ.get("MEMWIRE_UI_AGENT_ID", "default")
    db_path = os.environ.get("MEMWIRE_UI_DB_PATH")
    token = os.environ.get("MEMWIRE_UI_TOKEN") or None
    allow_unauth = os.environ.get("MEMWIRE_UI_ALLOW_UNAUTHENTICATED_PUBLIC", "") in {
        "1",
        "true",
        "yes",
        "on",
    ...
    try:
        ...
        print(f"[memwire-ui] FATAL: {exc}", file=sys.stderr)
        sys.exit(1)
