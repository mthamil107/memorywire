"""ASGI middleware for the AMP Governance UI: bearer-token auth + CSRF.
...
* Bypass: when an ``Authorization: Bearer`` header is present, CSRF is
  ...
  bots, the AMP CLI itself) cannot keep a cookie jar across requests,
...
CSRF_COOKIE_NAME: Final[str] = "amp_ui_csrf"
SESSION_COOKIE_NAME: Final[str] = "amp_ui_session"
...
def warn_if_public_host() -> None:
    ...
    no ``AMP_UI_TOKEN`` was supplied. Called from ``create_app`` so the
    warning surfaces on every boot, including in tests if relevant.
    """
    host = os.environ.get("AMP_UI_HOST", "")
    if host and host not in {"127.0.0.1", "localhost", "::1"}:
        print(
            f"[amp-ui] WARNING: AMP_UI_HOST={host!r} but AMP_UI_TOKEN is not set; "
...
class BearerAuthMiddleware:
    ...
    the ``amp_ui_session=<token>`` cookie (for browser flows). On any
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        response = PlainTextResponse(
            "unauthorized",
            status_code=401,
            headers={"WWW-Authenticate": 'Bearer realm="amp-ui"'},
        )
