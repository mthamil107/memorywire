...
AMP is in early v0 development. Only the most recent published version on PyPI
...
- The affected AMP version(s) and backend adapter(s) if known.

## Scope

In scope:

- The AMP reference implementation in `src/amp/`.
...
- Vulnerabilities in the surrounding application's authentication or transport
  security — AMP delegates these to the host application by design.
