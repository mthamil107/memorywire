"""``memwire`` command-line entry point â€” full Phase 5 implementation.

Three subcommands, all dispatched through the :class:`memwire.api.Memory`
facade so the CLI exercises the same code path SDK consumers use:

* ``memwire remember <content>`` â€” write a memory; prints ``id=<id>``.
* ``memwire recall <query>``     â€” read memories; prints a table or JSON.
* ``memwire forget``             â€” delete memories; prints ``forgotten=<n>``.

Common flags:

* ``--agent AGENT`` (default ``memwire-cli``) â€” scope every operation.
* ``--store URL``  (repeatable, default ``sqlite-vec://./memwire-cli.db``).
...
def _build_parser() -> argparse.ArgumentParser:
    """Construct the top-level ``memwire`` :class:`argparse.ArgumentParser`."""
    parser = argparse.ArgumentParser(
        prog=_PROG_NAME,
        description=f"{_PROG_NAME} {__version__} â€” memwire CLI",
    )
    parser.add_argument(
        "-V",
        "--version",
...
def _add_common_flags(sub: argparse.ArgumentParser) -> None:
    ...
    sub.add_argument(
        ...
        help="agent_id scope (default: memwire-cli).",
    )
    sub.add_argument(
        "--store",
        dest="stores",
        action="append",
        default=None,
        help="Store URL. Repeat for multi-store recall (default: sqlite-vec://./memwire-cli.db).",
    )
...
async def _handle_remember(args: argparse.Namespace) -> int:
    """Implement ``memwire remember <content>``."""
...
async def _handle_recall(args: argparse.Namespace) -> int:
    """Implement ``memwire recall <query>``."""
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run the ``memwire`` CLI. Returns the process exit code."""
    ...
    if handler is None:
        ...
        print(f"memwire: unknown command: {args.command}", file=sys.stderr)
        return 1

    try:
        return asyncio.run(handler(args))
    except _UserError as exc:
        print(f"memwire: {exc}", file=sys.stderr)
        return 1
        ...
        print(f"memwire: backend error: {exc}", file=sys.stderr)
        return 2
