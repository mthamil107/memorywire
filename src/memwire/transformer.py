...
Design notes
------------
* The transformer is *not* itself a :class:`memwire.store.MemoryStore` â€” it is a
  ...
  individual store or a :class:`memwire.router.MemoryRouter` works.
...
from memwire.models import MemoryType, RememberRequest, RememberResponse
...
class STMItem(BaseModel):
    ...
    # Mirror the memwire-wide model config (forward-compatible extras, alias
...
class STMToLTMTransformer:
    ...
    ----------
    target:
        Anything implementing :class:`memwire.store.MemoryStore`'s ``remember``
        method. Typically a :class:`memwire.router.MemoryRouter`.
    cadence_seconds:
