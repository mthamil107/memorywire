...
This adapter is the default local backend for the memwire reference
implementation. It implements every operation in the
:class:`memwire.store.MemoryStore` Protocol on top of a single SQLite database
...
* ``memories.last_recalled_at`` column added â€” required to honor
  ...
  :attr:`memwire.store.Capability.RECALL_TRACKING` claim.
...
class SqliteVecStore:
    ...
    def _init_schema(self) -> None:
        """Create the memwire storage schema if it does not already exist."""
        ddl = [
    ...
    def _parse_fsm(content: str) -> dict[str, Any]:
        ...
        in :class:`memwire.models.ProcedureSpec`. Non-object payloads still get a
    ...
    def _merge_sync(self, req: MergeRequest) -> MergeResponse:
        ...
        the canonical and duplicate strings may be either memwire memory ids or
