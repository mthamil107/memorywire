"""Cognee backend adapter for memwire (Phase 3).

This module exposes :class:`CogneeStore`, a :class:`memwire.store.MemoryStore`
implementation that translates memwire operations onto the public ``cognee``
...
* Cognee scopes data by *dataset name* (a string, defaulting to
  ...
  slot. memwire's ``agent_id`` is a different namespace (logical
...
* Cognee's ``add`` accepts a *string* of arbitrary text. There is no
  ...
  metadata via LLM extraction inside ``cognify``). memwire fields that don't
  map natively (``type``, ``confidence``, ``source``, ``expires_at``,
  caller metadata) are encoded as a **structured memwire header line**
...
* Cognee's ``add`` is not a per-record write â€” it returns ``None`` and
  ...
  hash id so memwire's :class:`RememberResponse` shape stays valid. v0.2
...
* Cognee's :meth:`forget` only deletes by ``data_id`` + ``dataset`` or
  ...
  primitive that maps cleanly onto memwire's id-list / filter contract.
...
* ``cognee://default`` is a reserved alias for the memwire default
  dataset name (``"memwire"``).
...
# bare ``cognee://default`` is scoped to the memwire project and does not
# collide with Cognee's own out-of-the-box ``main_dataset``.
_DEFAULT_DATASET = "memwire"

# The memwire overlay is serialised as a single JSON line and prepended to the
...
def _now_ms() -> int:
    """Return Unix epoch milliseconds â€” the memwire wire timestamp format."""
...
def _encode_amp_payload(
    *,
    ...
    """Build the structured memwire overlay carried as a header line.

    Returned dict is JSON-serialisable; keys mirror the memwire wire fields so
...
def _wrap_content(payload: dict[str, Any], content: str) -> str:
    """Prepend the memwire overlay header to the raw content."""
...
def _unwrap_content(blob: str) -> tuple[dict[str, Any], str]:
    ...
    memwire header (data ingested outside the adapter) returns an empty
...
def _entry_to_overlay_and_text(entry: Any) -> tuple[dict[str, Any], str, float | None]:
    ...
    the memwire overlay produced by :func:`_unwrap_content`.
    """
    if entry is None:
        return {}, "", None
    ...
    # it into the overlay as a best-effort. The memwire-header overlay wins
...
class CogneeStore:
    """memwire adapter for the ``cognee`` Python SDK.
    ...
    dataset:
        ...
        the memwire dataset alias ``"memwire"``; pass another name for
    ...
    â€” :class:`memwire.store.MemoryStore` is a ``@runtime_checkable`` Protocol
    ...
    def from_url(cls, url: str, *, client: Any | None = None) -> CogneeStore:
        ...
        is the reserved alias for the memwire default dataset name
        (``"memwire"``); any other host slot is treated verbatim as the
        dataset name.
    ...
    def capabilities(self) -> set[str]:
        """Capabilities Cognee supports under memwire semantics.

        * ``semantic`` / ``episodic`` â€” Cognee ingests any natural-language
          text; memwire type tags ride in the JSON header line and are
        ...
        * ``graph`` â€” Cognee is graph-database-backed (Kuzu / Neo4j) and
        ...
        Cognee does **not** offer procedural-FSM contracts under memwire
    ...
    async def remember(self, req: RememberRequest) -> RememberResponse:
        ...
        memwire fields Cognee doesn't natively model (``type``,
        ...
        write time and stashes it inside the memwire header â€” :meth:`recall`
    ...
    async def recall(self, req: RecallRequest) -> RecallResponse:
        """Retrieve memories via Cognee's ``search``/``recall`` and map to memwire hits.
        ...
        the memwire-header ``type``) and ``req.fresher_than_days`` (against
        the memwire-header ``created_at``).
        ...
        for raw in raw_results:
            ...
            # (or outside this adapter) carry no memwire header â€” surface
            ...
            # Re-expose the memwire overlay so consumers can read confidence
    ...
    async def expire(self, req: ExpireRequest) -> ExpireResponse:
        ...
        * ``archive`` â€” re-create the row via :meth:`remember` with the
          memwire-header ``archived`` flag set, then drop the original.
        ...
        * ``demote`` â€” re-create with ``confidence * 0.25`` baked in,
        ...
        # whatever the memwire request supplies indirectly via the agent
        ...
        for raw in raw_results:
            ...
            # corpora that lack an memwire header.
            mem_id = overlay.get("id")
            if not isinstance(mem_id, str) or not mem_id:
                continue
            # agent_id scoping.
