"""mem0 backend adapter for memwire (Phase 3).

This module exposes :class:`Mem0Store`, a :class:`memwire.store.MemoryStore`
implementation that translates memwire operations onto the public ``mem0``
...
* mem0's public client is **synchronous**. Every method on this adapter
  awaits ``anyio.to_thread.run_sync`` so the memwire async surface stays honest.
...
# memwire fields that mem0 does not understand natively are flattened into
...
def _to_epoch_ms(value: Any) -> int | None:
    ...
    or integer epoch seconds. memwire requires Unix epoch *milliseconds*. Unknown
...
class Mem0Store:
    """memwire adapter for the ``mem0`` Python SDK.
    ...
    :class:`memwire.store.MemoryStore` is a ``@runtime_checkable`` Protocol, and
    ...
    def capabilities(self) -> set[str]:
        """Capabilities mem0 supports under memwire semantics.
        ...
        * ``vector`` â€” mem0 ships its own vector store under the hood.
        ...
        not the memwire procedural-FSM contract â€” different shape), graph hops,
    ...
    def _record_matches_filter(record: dict[str, Any], flt: dict[str, Any]) -> bool:
        """Apply a flat memwire filter dict to a mem0 record returned by ``get_all``.
        ...
        special-case that the memwire ``type`` field maps to the
    ...
    async def remember(self, req: RememberRequest) -> RememberResponse:
        ...
        memwire fields mem0 doesn't natively support (``type``, ``confidence``,
        ...
        (returns ``{"results": [...]}``). memwire v0 only surfaces the *first*
        ...
        if principal is None:
            # mem0 itself would raise here, but the memwire error is friendlier.
            raise ValueError("mem0 adapter requires either `user_id` or `agent_id` on the request")

        # Merge caller-supplied metadata with the memwire-specific overlay.
        ...
        # memwire response shape stays valid; downstream audit can detect the
    ...
    async def recall(self, req: RecallRequest) -> RecallResponse:
        """Retrieve memories via ``mem0.search`` and map to memwire hits.
        ...
        # mem0 uses ``top_k`` (default 20); memwire uses ``k`` (default per spec
        ...
        for raw in raw_results:
            ...
            # Resolve the memwire type from the metadata tag the adapter writes
        ...
        # behaviour at the memwire-router layer; mem0's internal pipeline is
    ...
    async def forget(self, req: ForgetRequest) -> ForgetResponse:
        ...
        is False (the memwire default), this adapter still performs a hard delete
