"""Letta backend adapter for memwire (Phase 3).

This module exposes :class:`LettaStore`, a :class:`memwire.store.MemoryStore`
implementation that translates memwire operations onto the official
...
* The Letta client (``letta_client.Letta``) is **synchronous**. Every method
  on this adapter awaits ``anyio.to_thread.run_sync`` so the memwire async
  surface stays honest.
* Letta's archival memory is per-agent: every passage is scoped to a Letta
  ``agent_id``. memwire's own ``agent_id`` is a different namespace â€” typically
...
* Letta's ``agents.passages.create`` accepts ``text``, ``tags``, and
  ``created_at`` â€” and crucially, **no free-form metadata field**. memwire
...
* Letta's archival API is delete-only (no soft-delete primitive). When
  ...
  same convention as :mod:`memwire.store.mem0_adapter`. The audit log (Phase
...
# memwire fields encoded as Letta tags. ``key:value`` form keeps the tag set a
# flat list[str] (Letta's only supported shape) while still letting the
# recall path parse the original memwire fields back out. Choose short prefixes
...
def _tags_to_amp_overlay(tags: list[str] | None) -> dict[str, Any]:
    """Parse an memwire-tag list back into a flat overlay dict.

    Inverse of :func:`_amp_overlay_to_tags`. Unrecognised tags are
    ignored. The returned dict carries the original memwire-field names
...
def _amp_overlay_to_tags(
    *,
    ...
    """Encode the memwire overlay onto a flat tag list for Letta.

    Letta does not surface a free-form metadata blob on its archival API
    (only ``tags: list[str]``). Every memwire-specific field is therefore
...
class LettaStore:
    """memwire adapter for the official ``letta-client`` SDK.
    ...
    :class:`memwire.store.MemoryStore` is a ``@runtime_checkable`` Protocol;
    ...
    def from_url(cls, url: str, *, client: Any | None = None) -> LettaStore:
        ...
        spec-gap: agent_id is *required* at the Letta layer but memwire's
    ...
    def capabilities(self) -> set[str]:
        """Capabilities Letta supports under memwire semantics.

        * ``semantic`` / ``episodic`` â€” Letta archival memory stores any
          string passage; memwire type tags are encoded in the passage tag
        ...
        * ``vector`` â€” Letta indexes every passage with an embedding and
        ...
        Letta does **not** offer a procedural-FSM contract under memwire
    ...
    async def remember(self, req: RememberRequest) -> RememberResponse:
        ...
        memwire fields Letta doesn't natively model (``type``, ``confidence``,
        ...
        server may split content into multiple passages on chunking). memwire
        ...
        # passages so the memwire response shape stays valid.
        if first_id is None:
    ...
    async def recall(self, req: RecallRequest) -> RecallResponse:
        """Retrieve passages via ``agents.passages.search`` and map to memwire hits.
        ...
        spec-gap: ``fusion_used`` is reported as ``"rrf"`` (the memwire
    ...
    async def forget(self, req: ForgetRequest) -> ForgetResponse:
        ...
        ``req.hard_delete`` is False (the memwire default), this adapter
        ...
        if req.filter:
            ...
            for record in records:
                ...
                # the passage dict and the parsed memwire overlay/metadata.
                matched = True
