"""Cognee backend adapter for AMP (Phase 3).

This module exposes :class:`CogneeStore`, a :class:`amp.store.MemoryStore`
implementation that translates AMP operations onto the public ``cognee``
...
* Cognee scopes data by *dataset name* (a string, defaulting to
  ...
  slot. AMP's ``agent_id`` is a different namespace (logical
...
* Cognee's ``add`` accepts a *string* of arbitrary text. There is no
  ...
  metadata via LLM extraction inside ``cognify``). AMP fields that don't
  map natively (``type``, ``confidence``, ``source``, ``expires_at``,
  caller metadata) are encoded as a **structured AMP header line**
...
* Cognee's ``add`` is not a per-record write — it returns ``None`` and
  ...
  hash id so AMP's :class:`RememberResponse` shape stays valid. v0.2
...
* Cognee's :meth:`forget` only deletes by ``data_id`` + ``dataset`` or
  ...
  primitive that maps cleanly onto AMP's id-list / filter contract.
...
* ``cognee://default`` is a reserved alias for the AMP default
  dataset name (``"amp"``).
...
# bare ``cognee://default`` is scoped to the AMP project and does not
# collide with Cognee's own out-of-the-box ``main_dataset``.
_DEFAULT_DATASET = "amp"

# The AMP overlay is serialised as a single JSON line and prepended to the
...
def _encode_amp_payload(
    *,
    ...
    """Build the structured AMP overlay carried as a header line.

    Returned dict is JSON-serialisable; keys mirror the AMP wire fields so
...
def _wrap_content(payload: dict[str, Any], content: str) -> str:
    """Prepend the AMP overlay header to the raw content."""
...
def _unwrap_content(blob: str) -> tuple[dict[str, Any], str]:
    ...
    AMP header (data ingested outside the adapter) returns an empty
...
def _entry_to_overlay_and_text(entry: Any) -> tuple[dict[str, Any], str, float | None]:
    ...
    the AMP overlay produced by :func:`_unwrap_content`.
    """
    if entry is None:
        return {}, "", None
    ...
    # it into the overlay as a best-effort. The AMP-header overlay wins
...
class CogneeStore:
    """AMP adapter for the ``cognee`` Python SDK.
    ...
    dataset:
        ...
        the AMP dataset alias ``"amp"``; pass another name for
    ...
    def from_url(cls, url: str, *, client: Any | None = None) -> CogneeStore:
        ...
        is the reserved alias for the AMP default dataset name
        (``"amp"``); any other host slot is treated verbatim as the
        dataset name.
    ...
    def capabilities(self) -> set[str]:
        """Capabilities Cognee supports under AMP semantics.

        * ``semantic`` / ``episodic`` — Cognee ingests any natural-language
          text; AMP type tags ride in the JSON header line and are
        ...
        * ``graph`` — Cognee is graph-database-backed (Kuzu / Neo4j) and
        ...
        Cognee does **not** offer procedural-FSM contracts under AMP
    ...
    async def remember(self, req: RememberRequest) -> RememberResponse:
        ...
        AMP fields Cognee doesn't natively model (``type``,
        ...
        write time and stashes it inside the AMP header — :meth:`recall`
    ...
    async def recall(self, req: RecallRequest) -> RecallResponse:
        """Retrieve memories via Cognee's ``search``/``recall`` and map to AMP hits.
        ...
        the AMP-header ``type``) and ``req.fresher_than_days`` (against
        the AMP-header ``created_at``).
        ...
        for raw in raw_results:
            ...
            # (or outside this adapter) carry no AMP header — surface
    ...
    async def expire(self, req: ExpireRequest) -> ExpireResponse:
        ...
        * ``archive`` — re-create the row via :meth:`remember` with the
          AMP-header ``archived`` flag set, then drop the original.
        ...
        * ``demote`` — re-create with ``confidence * 0.25`` baked in,
        ...
        # whatever the AMP request supplies indirectly via the agent
        ...
        for raw in raw_results:
            ...
            # corpora that lack an AMP header.
            mem_id = overlay.get("id")
            if not isinstance(mem_id, str) or not mem_id:
                continue
            # agent_id scoping.
