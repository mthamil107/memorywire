...
This module exposes :class:`PgVectorStore`, a :class:`amp.store.MemoryStore`
implementation backed by PostgreSQL with the ``pgvector`` extension. It is the
production-grade analogue to :class:`amp.store.sqlite_vec.SqliteVecStore`:
...
* asyncpg is natively async; no thread-pool wrapping is needed, unlike
  :mod:`amp.store.sqlite_vec` / :mod:`amp.store.mem0_adapter`. Every method on
...
* The default embedder mirrors :mod:`amp.store.sqlite_vec`: 384-d output from
...
* IDs use :func:`uuid.uuid4` hex — same choice and same deferral note as
  :mod:`amp.store.sqlite_vec` (uuid7 once Python 3.14 is the floor).
...
class PgVectorStore:
    """AMP adapter for PostgreSQL + ``pgvector``.
    ...
    schema:
        Postgres schema namespace under which AMP tables live. Defaults to
        ``"amp"``. Must match ``[A-Za-z_][A-Za-z0-9_]*``.
    embedding_dim:
        Vector length the embedder produces. Defaults to 384 to match
        sentence-transformers' all-MiniLM-L6-v2 (same as
        :mod:`amp.store.sqlite_vec`).
    embedder:
    ...
    async def _resolve_entity(self, conn: Any, agent_id: str, key: str) -> list[Any]:
        ...
        Mirrors :class:`amp.store.sqlite_vec.SqliteVecStore._resolve_entity`.
        """
        rows = await conn.fetch(
