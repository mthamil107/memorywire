"""The :class:`MemoryRouter` — a :class:`amp.store.MemoryStore` composed of N child stores.

The router is the centrepiece of AMP's "any-backend" promise (spec §5). It
...
itself satisfies the :class:`amp.store.MemoryStore` Protocol, routers can
...
* Graph-hop boost dispatches to stores implementing the
  ...
  no store declares :attr:`amp.store.Capability.GRAPH`.
"""
