...
Design notes
------------
* The transformer is *not* itself a :class:`amp.store.MemoryStore` — it is a
  ...
  individual store or a :class:`amp.router.MemoryRouter` works.
...
from amp.models import MemoryType, RememberRequest, RememberResponse
from amp.store.base import MemoryStore
...
class STMItem(BaseModel):
    ...
    # Mirror the AMP-wide model config (forward-compatible extras, alias
...
class STMToLTMTransformer:
    ...
    ----------
    target:
        Anything implementing :class:`amp.store.MemoryStore`'s ``remember``
        method. Typically a :class:`amp.router.MemoryRouter`.
    cadence_seconds:
