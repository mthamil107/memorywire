...
This directory ships the memorywire Governance UI as a public hosted demo at
`https://memorywire-governance-demo.fly.dev`. The intent is a one-click "Try it
...
`python -m memorywire_ui`. The database lives on a persistent 1 GB volume
(`memorywire_demo_data`) so the seeded state survives restarts.

`MEMORYWIRE_UI_TOKEN` gates write operations (approve / reject / co-memorize /
...
`MEMORYWIRE_UI_TOKEN` is also **required** by the boot guard whenever
`MEMORYWIRE_UI_HOST` is non-loopback (`fly.toml` binds `0.0.0.0`). If the
...
`MEMORYWIRE_UI_ALLOW_UNAUTHENTICATED_PUBLIC=1` only if you intentionally
...
fly volumes create memorywire_demo_data --region iad --size 1
fly secrets set \
  MEMORYWIRE_UI_TOKEN=$(openssl rand -hex 32) \
  MEMORYWIRE_UI_CSRF_SECRET=$(python -c 'import secrets, base64; print(base64.b64encode(secrets.token_bytes(32)).decode())')
fly deploy --config deploy/fly.toml
```

If `memorywire-governance-demo` is taken on Fly (app names are global), edit
...
curl -fsS https://memorywire-governance-demo.fly.dev/ | head -n 5
...
```sh
fly secrets set MEMORYWIRE_UI_TOKEN=$(openssl rand -hex 32)              # rolling restart, ~10s
fly secrets set MEMORYWIRE_UI_CSRF_SECRET=$(python -c 'import secrets, base64; print(base64.b64encode(secrets.token_bytes(32)).decode())')
```

Setting a secret triggers a rolling redeploy automatically. Rotating
`MEMORYWIRE_UI_CSRF_SECRET` invalidates every active browser cookie, which
...
* Per-session `agent_id` scoping Ã¢â‚¬â€ the demo is hard-pinned to
  `demo-agent`.
* Read-only banner in the UI when `MEMORYWIRE_UI_TOKEN` is set and no session
